/*
 * planetScript.js — Welcome → Overview → Mercury focus
 * Requires Three.js + GSAP. Exposes:
 *   - window.initSolarSystem()
 *   - window.startWelcomeSequence()   // brief zoomed-out overview → auto-focus Mercury
 */
...
(function () {
  ...
  let focusTargets = [], focusIndex = 0, focusOffset = new THREE.Vector3(0, 10, 60);
  ...
  window.startWelcomeSequence = function startWelcomeSequence() {
    // 1) Ensure camera starts zoomed out, show the whole system
    camera.position.set(0, 220, 800);
    camera.lookAt(0, 0, 0);
    ...
    // optional gentle dolly in during overview
    gsap.to(camera.position, { z: 640, duration: 2.2, ease: "power1.out" });

    // 2) After a short pause, glide to Mercury (index 1)
    const goFirst = () => {
      focusOn(1); // Mercury
      if (skipBtn) {
        skipBtn.hidden = true;
    ...
    // Skip button jumps immediately
    if (skipBtn) {
      skipBtn.onclick = goFirst;
    }

    // Auto-advance after 2.4s
    gsap.delayedCall(2.4, goFirst);
  };

  // ===== Focus the camera on a target (Sun=0, Mercury=1, …) and lock on =====
  ...
  function animate() {
    ...
    // Keep camera looking at the current target (smoothly)

(function () {
  ...
  function makeStars(count) {
    ...
    const mat = new THREE.PointsMaterial({ size: 0.8, sizeAttenuation: true, color: 0xffffff, transparent: true, opacity: 0.9 });
  ...
  window.initSolarSystem = function initSolarSystem() {
    ...
    PLANETS.forEach((cfg, i) => {
      const mat = new THREE.MeshStandardMaterial({ map: loader.load(cfg.tex), roughness: 1, metalness: 0 });
      const mesh = new THREE.Mesh(new THREE.SphereGeometry(cfg.size, 64, 64), mat);
      mesh.userData = { ...cfg, angle: i * 0.8 + 0.3, baseRot: 0.003 + Math.random() * 0.01 };
